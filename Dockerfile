...
# Use db:schema:load which doesn't require eager loading
RUN echo '#!/bin/sh' > /app/docker-entrypoint.sh && \
    echo 'set -e' >> /app/docker-entrypoint.sh && \
    echo 'echo "=== Loading database schema ==="' >> /app/docker-entrypoint.sh && \
    echo 'RAILS_ENV=production bundle exec rails db:schema:load 2>&1 || echo "Schema already loaded or migration needed"' >> /app/docker-entrypoint.sh && \
    echo 'echo "=== Running migrations ==="' >> /app/docker-entrypoint.sh && \
    echo 'RAILS_ENV=production bundle exec rails db:migrate 2>&1 || echo "Migrations completed"' >> /app/docker-entrypoint.sh && \
